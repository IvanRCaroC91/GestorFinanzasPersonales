...
// Endpoints que requieren headers especiales
...
// Verifica si el endpoint es protegido
...
axiosInstance.interceptors.request.use(
    (config) => {
        ...
        console.log(
            `[API] ${config.method?.toUpperCase()} ${url} | Auth: ${
                shouldAddAuth ? 'YES' : 'NO'
            } | UserId: ${authService.getUserId() || 'NONE'}`
        );
